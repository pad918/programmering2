defmodule Day16 do
  ...
  def task(t) do
    ...
    open_taps = Map.new(Enum.filter(parsed_map,
      fn(x) ->
        {_, {rel, _}} = x
        rel != 0
      end
    ...
    #parsed
    best = optimize_path(:AA, [], parsed_map, t, open_taps, %{})
    {moves, score, _} = best
    {score, moves}
  end
  ...
  def search_path(curr, moves, graph, time_left, open_taps, cache) do
    ...
    turn_on_tap_move =
    if(Map.has_key?(open_taps, curr)) do
      optimize_path(curr, [{:on, curr}|moves], graph, time_left-1, Map.put(open_taps, curr, curr), cache)
    else
      {[], 0, cache}
    end
    ...
    best_move = Enum.reduce(legal, {[], nil, -10, new_cache},
      fn(kran, acc) ->
        {_, _, _, new_cache} = acc
        {best_moves, value, new_cache} = optimize_path(kran, [{:move, kran}|moves], graph, time_left-1, open_taps, new_cache)
  ...
  def optimize_path(curr, moves, graph, time_left, open_taps, cache) do
    ...
    if(cached_result != nil) do
      ...
      search_path(curr, moves, graph, time_left, open_taps, cache)
    end
  end
